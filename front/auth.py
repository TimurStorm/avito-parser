...
def reg(email, password, username):
    ...
    resp = post(f"http://localhost:443/reg?email={email}&password={pwd}&username={username}")
    info = loads(resp.content.decode("utf-8"))
    if resp.status_code == 200:
        set_password(service_name="Parser", username=USERNAME + "_pwd", password=password)
...
def login(email, password):
    ...
    print(info["text"])
    if resp.status_code == 200:
        if get_password(service_name="Parser", username=USERNAME + "_pwd") is None or \
                get_password(service_name="Parser", username=USERNAME + "_ema") is None:
            set_password(service_name="Parser", username=USERNAME + "_pwd", password=password)
            set_password(service_name="Parser", username=USERNAME + "_ema", password=email)
    return info
